        ...
        {tabs.map((tab) => (
          <Tooltip key={tab.id} content={tab.label} side="top" className="flex-1">
            <button
              ...
              className={`flex h-11 w-full items-center justify-center gap-2 rounded-xl px-4 text-xs font-semibold outline-none transition cursor-pointer sm:text-sm ${value === tab.id ? "shadow-sm" : "text-slate-500 hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-white/5"}`}
