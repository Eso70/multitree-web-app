        ...
        {canUpgrade && (
          <Tooltip content="پلانەکەت بەرز بکەرەوە" side="right" disabled={!collapsed}>
            <button
              type="button"
              onClick={onUpgrade}
              className={`mt-3 flex w-full shrink-0 items-center rounded-xl border border-transparent px-3 py-2.5 text-sm font-semibold text-[var(--theme-ink)] shadow-sm transition [background:var(--theme-css)] hover:brightness-95 disabled:cursor-wait disabled:opacity-60 cursor-pointer ${collapsed ? "md:mt-0 md:justify-center md:px-0" : "gap-3"}`}
            >
