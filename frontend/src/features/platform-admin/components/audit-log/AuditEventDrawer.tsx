    ...
    <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-3 py-2 last:border-0 dark:border-white/5">
      ...
      <div className="flex min-w-0 items-center gap-2" dir="ltr">
        <Tooltip content={value} side="top">
          <span className="truncate font-mono text-xs text-slate-700 dark:text-slate-300 cursor-default">{value}</span>
        </Tooltip>
...
function CopyButton({ value }: { value: string }) {
  ...
  return (
    <Tooltip content={copied ? "کۆپی کرا" : "کۆپیکردن"} side="top">
      <button
        type="button"
        onClick={async () => {
          ...
          if (success) {
            ...
            toast.error("کۆپیکردن سەرکەوتوو نەبوو");
          }
        }}
        className="rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-white/10 dark:hover:text-white cursor-pointer"
        aria-label="کۆپیکردن"
      >
